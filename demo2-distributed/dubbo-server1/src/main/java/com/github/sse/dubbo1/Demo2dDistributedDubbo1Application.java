...
public class Demo2dDistributedDubbo1Application {
    private static final Logger log = LoggerFactory.getLogger(Demo2dDistributedDubbo1Application.class);
    @Resource
...
//                        sendByUserId(distributedConnectionService.qos(), users);
    ...
    private static <RESPONSE> RESPONSE sendByUserId(SendService<RESPONSE> sendService, List<MyAccessUser> users) {
        ...
        return sendService.sendByUserId(users.stream().map(MyAccessUser::getId).collect(Collectors.toList()),
                "server-push", "{\"name\":\"sendByUserId dubbo1号推送的\"}");
    }

    public static void main(String[] args) {
        SpringApplication.run(Demo2dDistributedDubbo1Application.class, args);
    }
